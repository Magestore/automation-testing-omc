<?php
/**
 * Copyright © 2013-2017 Magento, Inc. All rights reserved.
 * See COPYING.txt for license details.
 */

namespace Magento\Giftvoucher\Test\TestCase\GiftcardProduct;

use Magento\Catalog\Test\Fixture\Category;
use Magento\Giftvoucher\Test\Fixture\Giftproduct;
use Magento\Giftvoucher\Test\Page\Adminhtml\GiftvoucherProductIndex;
use Magento\Giftvoucher\Test\Page\Adminhtml\GiftvoucherProductNew;
use Magento\Mtf\TestCase\Injectable;

/**
 * Test Creation for CreateVirtualProductEntity
 *
 * Test Flow:
 * 1. LoginTest as admin.
 * 2. Navigate to the Products > Inventory > Catalog.
 * 3. Click on "+" dropdown and select Virtual Product type.
 * 4. Fill in all data according to data set.
 * 5. Save product.
 * 6. Verify created product.
 *
 * @group Virtual_Product_(CS)
 * @ZephyrId MAGETWO-23417
 */
class CreateGiftVoucherProductEntityTest extends Injectable
{
    /* tags */
    const TEST_TYPE = 'acceptance_test, extended_acceptance_test';
    const MVP = 'no';
    const DOMAIN = 'MX';
    /* end tags */

    /**
     * Category fixture
     *
     * @var Category
     */
    protected $category;

    /**
     * Product page with a grid
     *
     * @var GiftvoucherProductIndex
     */
    protected $productGrid;

    /**
     * Page to create a product
     *
     * @var GiftvoucherProductNew
     */
    protected $newProductPage;

    /**
     * Prepare data
     *
     * @param Category $category
     * @return array
     */
    public function __prepare(Category $category)
    {
        $category->persist();
        return [
            'category' => $category
        ];
    }

    /**
     * Injection data
     *
     * @param GiftvoucherProductIndex $productGrid
     * @param GiftvoucherProductNew $newProductPage
     * @return void
     */
    public function __inject(GiftvoucherProductIndex $productGrid, GiftvoucherProductNew $newProductPage)
    {
        $this->productGrid = $productGrid;
        $this->newProductPage = $newProductPage;
    }

    /**
     * Run create product virtual entity test
     *
     * @param Giftproduct $product
     * @param Category $category
     * @return void
     */
    public function testCreate(Giftproduct $product, Category $category)
    {
        // Steps
        $this->productGrid->open();
        $this->productGrid->getGridPageActionBlock()->addProduct('giftvoucher');
        $this->newProductPage->getProductForm()->fill($product, null, $category);
        $this->newProductPage->getFormPageActions()->save();
    }
}
