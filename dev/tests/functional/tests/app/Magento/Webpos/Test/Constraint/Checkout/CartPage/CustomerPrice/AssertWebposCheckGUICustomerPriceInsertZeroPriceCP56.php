...
/**
 * Class AssertWebposCheckGUICustomerPriceInsertZeroPriceCP56
 * @package Magento\Webpos\Test\Constraint\Checkout\CartPage\CustomerPrice
 */
class AssertWebposCheckGUICustomerPriceInsertZeroPriceCP56 extends AbstractConstraint
{
    public function processAssert(WebposIndex $webposIndex, $amountValue, $product, $price)
    {
        \PHPUnit_Framework_Assert::assertEquals(
            ...
            'Cart page - After set value to amount input with a zero value it must be create more original price. But the original price is not visible correctly.'
        );
        \PHPUnit_Framework_Assert::assertTrue(
            $webposIndex->getCheckoutCartItems()->getCartItemPrice($product->getName())->isVisible(),
            'Cart page - After set value to amount input with a zero value it must be create more original price. But the original price is not visible.'
        );
    }
